// Implements automatic scanning and common functions
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <time.h>
...
int pak_wifi_request_connection(struct PakNet *ctx, struct PakWiFiApFilter *spec, int (*cb)(struct PakNet *ctx, struct PakWiFiAdapter *, void *arg), void *arg) {
	...
	if ((adapter = pak_wifi_get_adapter(ctx, 0)) == NULL) {
	...
	if ((current_ap = pak_wifi_get_connected_ap(ctx, adapter)) != NULL) {
	...
	for (int i = 0; i < n_aps; i++) {
		if ((ap = pak_wifi_get_ap(ctx, adapter, i)) == NULL) {
